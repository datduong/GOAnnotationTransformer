...
choice='2embPpiAnnotE256H1L12I512Set0/ProtAnnotTypeLarge16Jan20/YesPpiAnd3DYesTypeScaleFreezeBert12Ep10e10Drop0.1' 
...
for ontology in mf cc bp ; do # 'cc' 'bp'
  ...
  train_data_file='/local/datdb/deepgo/dataExpandGoSet16Jan2020/train/fold_1/ProtAnnotTypeData/train-'$ontology'-input-bonnie-concat.tsv' ## okay to call it as long as it has ppi
  eval_data_file='/local/datdb/deepgo/dataExpandGoSet16Jan2020/train/fold_1/ProtAnnotTypeData/dev-'$ontology'-input-bonnie-concat.tsv'
  ...
  for test_data in 'test'  ; do # 'dev'
    ...
    # CUDA_VISIBLE_DEVICES=7 python3 -u RunTokenClassifyProtData.py --model_name_or_path $model_name_or_path --new_num_labels $new_num_labels --save_prediction $save_prediction --cache_name $cache_name --block_size $block_size --mlm --bert_vocab $bert_vocab --train_data_file $train_data_file --output_dir $output_dir --per_gpu_eval_batch_size $batch_size --config_name $config_name --do_eval --model_type $model_type --overwrite_output_dir --evaluate_during_training --eval_data_file $eval_data_file --label_2test $label_2test --config_override --eval_all_checkpoints --checkpoint $checkpoint --pretrained_label_path $pretrained_label_path --aa_type_file $aa_type_file --reset_emb_zero > $output_dir/'eval_'$test_data'_expand.txt'
  done
