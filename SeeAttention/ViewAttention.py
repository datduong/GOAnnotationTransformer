...
def evaluate(args, model, tokenizer, label_2test_array, prefix="", config=None, entropy_loss_weight=None, protein_name=None):
  ...
  for batch_counter, batch in tqdm(enumerate(eval_dataloader), desc="Evaluating"): ####
    ...
    if len( set(protein_name[row_counter:end]).intersection( set(list_prot_to_get) ) ) == 0 :
      ## suppose we don't enter this @if, then we update @row_counter at end, before we call "@for batch_counter...."
    ...
    for k in attention_value:
      out = {}
      out[k] = attention_value[k] ## seem stupid, but we need this format later
      if len(out[k])!=12:
        print ('fail {}'.format(k))
      else:
        pickle.dump(out, open(os.path.join(args.output_dir,"SeeAttention/"+k+"_attention_value.pickle"), 'wb') )
...
def main():
  ...
  parser.add_argument("--name_get_attention", type=str, default=None)
  ...
  protein_name = pd.read_csv(args.eval_data_file, dtype=str, sep="\t", header=None)
  ...
  label_2test_array = label_2test_array.reset_index(drop=True) ## otherwise get wrong indexing
  ...
  if args.new_num_labels is not None:
    result = evaluate(args, model, tokenizer, label_2test_array, prefix='zeroshot', config=config, entropy_loss_weight=entropy_loss_weight, protein_name=protein_name)
    return result
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    ...
    for checkpoint in checkpoints:
      ...
      print ('model.bert.encoder.output_attentions')
      print (model.bert.encoder.output_attentions)
      model.to(args.device)
      result = evaluate(args, model, tokenizer, label_2test_array, prefix=global_step, config=config, entropy_loss_weight=entropy_loss_weight, protein_name=protein_name)
