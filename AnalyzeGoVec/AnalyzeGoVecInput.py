...
# onto_type_dict_chkpt = {'mf': 90594, 'bp': 116144, 'cc': 127647}
onto_type_dict_chkpt = {'mf': 60480, 'cc': 92417, 'bp': 65475} ##!! checkpoint

for onto_type,onto_checkpoint in onto_type_dict_chkpt.items() :
  ...
  os.chdir('/local/datdb/deepgo/data/BertNotFtAARawSeqGO/'+onto_type+'/fold_1/2embPpiAnnotE256H1L12I512Set0/NoPpiNoTypeScaleFreezeBert12Ep10e10Drop0.1/checkpoint-'+str(onto_checkpoint))
  ...
  GOvec = model_params['bert.embeddings_label.word_embeddings.weight'].cpu().data.numpy() ##!!##!!##!! this is the input layer, so it is the same as pre-trained. 

  model_params = None ## clear from GPU
  print (GOvec.shape) ## num_label x 256 dim for each go label
