...
choice='2embPpiGeluE768H6L8I768PretrainLabelDrop0.2'
for ontology in 'mf' ; do
  ...
  CUDA_VISIBLE_DEVICES=1 python3 -u run_token_classify_2emb_ppi.py --block_size 1792 --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --num_train_epochs 100 --per_gpu_train_batch_size 2 --per_gpu_eval_batch_size 6 --config_name $config_name --do_train --model_type bert --overwrite_output_dir --save_steps 10000 --logging_steps 10000 --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --learning_rate 0.0001 --seed 2019 --fp16 --pretrained_label_path $pretrained_label_path > $output_dir/train_point.txt # --no_cuda
  ...
  for test_data in 'dev' 'test'; do
    ...
    CUDA_VISIBLE_DEVICES=1 python3 -u run_token_classify_2emb_ppi.py --block_size 1792 --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --num_train_epochs 50 --per_gpu_eval_batch_size 10 --config_name $config_name --do_eval --model_type bert --overwrite_output_dir --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --eval_all_checkpoints --fp16 --pretrained_label_path $pretrained_label_path > $output_dir/'eval_'$test_data'_check_point.txt'
  done 
