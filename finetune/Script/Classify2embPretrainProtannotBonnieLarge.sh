...
block_size=2716 # mf and cc 1792 but bp has more term  2048
...
for ontology in 'cc' ; do # 'cc' 'bp'
  ...
  last_save=$server/'deepgo/dataExpandGoSet/BertNotFtAARawSeqGO/'$ontology/'fold_1'/$choice
  output_dir=$server/'deepgo/dataExpandGoSet/BertNotFtAARawSeqGO/'$ontology/'fold_1'/$choice
  mkdir $output_dir

  bert_vocab=$output_dir/'vocabAA.txt'
  config_name=$output_dir/config.json

  aa_type_file='/local/datdb/deepgo/dataExpandGoSet/fold_1/ProtAnnotTypeData/train_'$ontology'_prot_annot_type_count.pickle'
  ...
  eval_masklm_data='/local/datdb/deepgo/dataExpandGoSet/train/fold_1/ProtAnnotTypeData/dev-'$ontology'-input-bonnie.tsv'
