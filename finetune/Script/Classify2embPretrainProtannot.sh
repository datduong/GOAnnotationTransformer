...
choice='2embPpiAnnotE768H1L12I768PreLabSet0/Ep10e10Drop0.1' # Lr5e-5 Dr0.2
...
for ontology in 'bp' ; do
  ...
  for test_data in 'test'; do # 'dev' 
    ...
    CUDA_VISIBLE_DEVICES=4 python3 -u RunTokenClassifyProtData.py --block_size $block_size --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --per_gpu_eval_batch_size 4 --config_name $config_name --do_eval --model_type bert --overwrite_output_dir --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --eval_all_checkpoints --fp16 --aa_type_emb --aa_type_file $aa_type_file > $output_dir/'eval_'$test_data'_check_point.txt'
