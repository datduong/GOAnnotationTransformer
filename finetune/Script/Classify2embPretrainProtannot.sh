...
choice='2embPpiAnnotE256H1L12I512Set0/YesPpiYesTypeScaleTrainBert12Ep10e10Drop0.1' # Lr5e-5 Dr0.2
model_type='ppi'
...
for ontology in 'mf' 'cc' 'bp' ; do
  ...
  #   CUDA_VISIBLE_DEVICES=7 python3 -u RunTokenClassifyProtData.py --cache_name $cache_name --block_size $block_size --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --per_gpu_eval_batch_size 8 --config_name $config_name --do_eval --model_type $model_type --overwrite_output_dir --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --eval_all_checkpoints --fp16 --checkpoint $checkpoint > $output_dir/'eval_'$test_data'_check_point.txt'
