...
choice='2embPpiAnnotE768H1L12I768PreLabSet0' # Lr5e-5 Dr0.2
...
for ontology in 'mf' ; do
  ...
  CUDA_VISIBLE_DEVICES=5 python3 -u RunTokenClassifyProtData.py --block_size $block_size --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --num_train_epochs 100 --per_gpu_train_batch_size 4 --per_gpu_eval_batch_size 2 --config_name $config_name --do_train --model_type bert --overwrite_output_dir --save_steps $save_every --logging_steps $save_every --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --learning_rate 0.0001 --seed 2019 --fp16 --aa_type_emb --aa_type_file $aa_type_file --reset_emb_zero > $output_dir/train_point.txt # --no_cuda
  ...
  for test_data in 'test'; do # 'dev' 
    ...
    CUDA_VISIBLE_DEVICES=5 python3 -u RunTokenClassifyProtData.py --block_size $block_size --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --per_gpu_eval_batch_size 4 --config_name $config_name --do_eval --model_type bert --overwrite_output_dir --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --eval_all_checkpoints --fp16 --aa_type_emb --aa_type_file $aa_type_file > $output_dir/'eval_'$test_data'_check_point.txt'
