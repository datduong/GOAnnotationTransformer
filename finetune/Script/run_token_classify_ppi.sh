...
for ontology in 'mf' 'cc' ; do 
  ...
  ## continue training 
  CUDA_VISIBLE_DEVICES=5 python3 -u run_token_classify_ppi.py --block_size 1792 --mlm --bert_vocab $bert_vocab --train_data_file $train_masklm_data --output_dir $output_dir --num_train_epochs 50 --per_gpu_train_batch_size 2 --per_gpu_eval_batch_size 8 --config_name $config_name --do_train --model_type bert --overwrite_output_dir --save_steps 10000 --logging_steps 10000 --evaluate_during_training --eval_data_file $eval_masklm_data --label_2test $label_2test --config_override --fp16 > $output_dir/train_point.txt # --no_cuda
