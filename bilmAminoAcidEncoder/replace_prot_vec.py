...
for data_type in ['test','train','dev']:
  for onto in ['cc','bp','mf']:
    ...
    map_vec ={}
    know_file = open("ProtAnnotTypeData/"+data_type+"-"+onto+"-input-bonnie.tsv","r")
    for line in know_file: ## no header
      line = line.split('\t')
      map_vec[line[0]] = re.sub(" ",";",line[-2]) ## 2nd to last
    know_file.close()
    ##
    ## now get open file to replace
    fin = open(data_type+"-"+onto+".tsv","r")
    fout = open(data_type+"-"+onto+"-bonnie.tsv","w")
