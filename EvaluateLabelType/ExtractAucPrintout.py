...
fout = open("/local/datdb/deepgo/data/train/fold_1/DeepGoZeroBase.test.txt","w")
...
for name in name_list: # 'BiLSTM', 'Bertd11d12', 'GCN', 'BertAve12' Base
  ...
  for onto in ['bp','mf','cc']: 
    try:
      fin = open("/local/datdb/deepgo/data/train/fold_1/DeepGoZeroBase"+name+"/NotNormalize/"+onto+"b32lr0.0005RMSprop/test_frequency.log","r")
    except:
      continue
    print ('\nmethod {} ontology {}\n'.format(name,onto))
    fout.write('\nmethod {} ontology {}\n'.format(name,onto))
